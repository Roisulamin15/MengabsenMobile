...
class SuratTugasFormController extends GetxController {
  ...
  File? pickedFile; // optional if you want to attach a file during submit (not required by backend for initial submission)

  Future<void> pickDate(BuildContext context) async {
    final date = await showDatePicker(context: context, firstDate: DateTime(2020), lastDate: DateTime(2030), initialDate: DateTime.now());
    if (date != null) {
  ...
  // optional if you want to attach a file when submitting (not required here)
  ...
  // Validasi wajib: tanggal, jam, bertemu_dengan, perusahaan, detail_kunjungan
  if (tanggalC.text.isEmpty || jamC.text.isEmpty || bertemuC.text.isEmpty || perusahaanC.text.isEmpty || detailC.text.isEmpty) {
    Get.snackbar("Error", "Lengkapi semua field wajib!", backgroundColor: Colors.red, colorText: Colors.white);
    return;
  }
  ...
  try {
    ...
    final res = await ApiService.submitSuratTugasBaru(payload, token);
    Get.back();
    ...
    if (res['status'] == true || res['status'] == 'success') {
      ...
      final id = res['data'] != null ? res['data']['id'] : null;
      Get.dialog(
        Dialog(
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                decoration: const BoxDecoration(color: Color(0xFFFF7A00), borderRadius: BorderRadius.vertical(top: Radius.circular(12))),
                child: const Center(child: Text("Berhasil Diajukan", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.white))),
              ),
              ...
              const Padding(
                ...
                child: Text("Permintaan berhasil diajukan\nSilahkan menunggu konfirmasi dari HRD.", textAlign: TextAlign.center),
              ),
              const SizedBox(height: 20),
              Padding(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    Get.back();
                    if (id != null) {
                      Get.off(() => const SuratTugasView(), arguments: {"id": id});
                    } else {
                      Get.offAllNamed("/surat_tugas");
                    }
                  },
                  style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFFF7A00), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)), padding: const EdgeInsets.symmetric(horizontal: 40, vertical: 12)),
      ...
      Get.snackbar("Gagal", res['message']?.toString() ?? "Gagal mengajukan");
    }
  } catch (e) {
    isSubmitting.value = false;
    Get.back();
